...
Авторизация в Telegram MTProto делается **один раз вручную** через GramJS. Session-string сохраняется в `.env` под ключом `TELEGRAM_SESSION_STRING` и переиспользуется всеми последующими импортами без интерактивных запросов.

**Скрипты:**
- `scripts/telegram-auth.mjs` — интерактивный one-shot: SMS-код + 2FA cloud password → session-string в `.env`. Запускать **только через `!` префикс в input-боксе Claude Code** или **через обычный терминал** (`node scripts/telegram-auth.mjs`). Через мой Bash-инструмент **не запускать** — он не пробрасывает stdin, скрипт зависнет.
- `scripts/telegram-test.mjs` — non-interactive верификация. Подключается с сохранённой сессией и печатает только derived facts (user_id, длины полей, булевы наличия, счётчики). Полезен для проверки после restart'а / переустановки пакетов.

**От пользователя требуется (один раз):**
- В `.env` выставлены `TELEGRAM_API_ID` (integer), `TELEGRAM_API_HASH` (получаются на https://my.telegram.org), `TELEGRAM_PHONE` (в международном формате с `+`)
- При запуске `auth.mjs` — ввести SMS-код и облачный 2FA-пароль в свой терминал

**Перевыпуск сессии:** просто запустить `auth.mjs` снова — старая строка в `.env` будет перезаписана новой. Перед записью создаётся `.env.backup` (в `.gitignore`), удаляется после успешного write; если запись упала — backup остаётся как страховка.

**Где живёт session-string:** строго в `.env`. Никогда в коде, никогда в БД, никогда в commits. Для production-деплоя — переменная окружения хостинга (Vercel, Railway, fly.io — все умеют env vars). Никаких `.session`-файлов на диске.

**Жёсткие правила для скриптов:**
- Никаких `console.log` самой session-string, SMS-кода, 2FA-пароля, api_hash. Только derived facts.
- Никаких `err.stack` в логах — Node может запечь аргументы (= секреты) в стек. Только `err.constructor.name` + категоризированное сообщение.
- Никаких тестовых сообщений в Saved Messages с содержанием секретов. (Сейчас тестовых сообщений нет вообще — sanity-check делает `getMe()` + `getDialogs(limit=1)`.)

## Day 3-A: API import logic

После авторизации Telegram-импорт делается через MTProto API (GramJS). Это **главный** источник identity для людей. JSON-экспорт идёт в дополнение (см. ниже).

**Источники identity** (`ContactIdentity.source`):
- `telegram` — реальный TG-аккаунт с user_id. `confidence='imported'`.
- `telegram_self` — мой собственный аккаунт. `confidence='imported'`.
- `telegram_addressbook` — JSON-only «исторический» контакт (нет user_id). `confidence='self_reported'`. `sourceId = sha256(phoneE164)`.
- `telegram_shared` — пересланный vCard через чат (День 3-Б).

**Discovery source** (`ContactIdentity.discoverySource`) — отслеживает «как мы узнали»:
- `contacts_api` — из `contacts.GetContacts` (формальный TG-контакт).
- `direct_chat` — из `users.GetUsers` для собеседника direct-чата, которого нет в адресной книге (Phase 5d).
- `json_addressbook` — только в JSON-экспорте, без живого TG-аккаунта.
- `self` — это я.
- `self_reported` — legacy vCard `X-SOCIALPROFILE`.

**Логика merge:**
- API возвращает `phone` → ищем PhoneNumber в БД → если есть, привязываем identity к этому Contact (merge с vCard). Иначе создаём новый Contact.
- Если `confidence='self_reported'` identity уже есть с тем же `handle` — promote: `sourceId` → user_id, `confidence` → `'imported'`, обогащаем rawData.
- Конфликты (один sourceId на разные Contact) — в `result.conflicts`, без автоматического merge. Ручное слияние в Дне 9.

**Phase 3 idempotency** через `ContactIdentity.enrichedAt`:
- NULL = `users.GetFullUser` не запускался для этой identity.
- Поле выставляется **на успех И на ошибку** (deleted/banned/restricted), чтобы не дёргать API повторно. На повторном запуске Phase 3 обрабатывает только `enrichedAt IS NULL`.

**Storage для фото:** `storage/telegram-photos/<photoFileId>.jpg`. `photoFileId = sha256(userId:photoId:dcId).slice(0,16)` — стабильный идентификатор, идемпотентный (если файл существует — не перекачиваем). Папка в `.gitignore` (содержимое 20-200 МБ).

**Rate-limit стратегия** (см. `src/lib/telegram/rate-limit.ts`):
- Per-call: до 3 retries при flood-wait ≤ 300 сек, со сном N+1.
- Per-call hard cap: flood-wait > 300 сек → re-throw (Telegram явно сигналит abuse).
- Job-level cumulative: суммарный flood-wait > `TELEGRAM_FLOOD_LIMIT_SEC` (default 1800 = 30 мин) → abort import. Для долгих прогонов (~70 мин с 130 flood-waits) используй `TELEGRAM_FLOOD_LIMIT_SEC=5400` (90 мин).
- Локальный throttle: 25 req/sec в скользящем окне 1 сек.

**Personal channels** (Premium-фича): `users.GetFullUser.personalChannelId` указывает на канал, привязанный к профилю. Импортируется как `Chat` с `linkedToContactId = ownerContactId`. На реальном дампе — 295 таких каналов.

**Phase 5d — discovery скипнутых direct-чатов:**
- Phase 5c набирает `skippedDirectUserIds[]` — собеседников direct-чатов, для которых ни в Phase 2 (contacts.GetContacts), ни в БД нет identity.
- Phase 5d батчами по 100 вызывает `users.GetUsers` → создаёт `ContactIdentity` с `discoverySource='direct_chat'` → enrichит через `enrichOneIdentity` → создаёт `ChatMember` для этого direct-чата.
- Большинство discovered users имеют phone скрытый (privacy), поэтому `mergedWithVcard` обычно близко к 0.

### Group/Chat architecture

**Contact ≠ ChatParticipant.** Это load-bearing разделение для будущих сессий:

1. **Contact = только люди** (которых ты осознанно знаешь). Никогда не создаются автоматически из участников групп / каналов.
2. **Chat = self-contained сущность** с богатыми метаданными (title, description, member_count, photo, usernames, slow_mode, creation_date). Может пересекаться с Contact через ChatMember **только для direct-чатов** (1:1 переписки и Saved Messages).
3. **Группы из любого source** (telegram / whatsapp / linkedin / slack / ...) идут в одну `Chat`-таблицу через `(source, sourceId)` unique. Источник переменчив, схема стабильна.
4. **День 3-A**: импортируем все `Chat` с метаданными (groups + supergroups + channels тоже), но `getParticipants` не вызываем. `ChatMember` создаётся только для type='direct' и type='self'.
5. **«День групп» (TBD)**: тогда же решим про участников. Возможные пути: `confidence='observed'` для увиденных в группах но не контактов; или отдельная таблица `GroupParticipant`. Сейчас зарезервирована пустая таблица `CommonChat` для аналитики «у этих двоих N общих групп».

### Day 3-A: Known limits

- **~1367 direct-чатов с неразрешимым counterpart user_id.** Из 3330 direct-чатов в API: 2107 покрыты `contacts.GetContacts`, 880 разрешены через Phase 5d, **1367 — deactivated/banned/inaccessible TG-аккаунты**. У них есть `Chat`-строка, но `ChatMember` для counterpart отсутствует. В День 3-Б при импорте сообщений мы будем видеть `from_id`, не привязанный к никому — нужно будет логировать число таких сообщений; если массово — пересмотрим стратегию (например, создавать «phantom» identity без contactId).
- **`mergedWithVcard = 0` в Phase 5d.** Все 880 discovered direct-chat users имеют phone скрытый настройками приватности. Это согласуется с гипотезой «implicit contacts (DM-only) ≠ phonebook contacts». Слияние с vCard для них в День 9 ручное.
- **Rate-limit cumulative budget**. На реальном дампе 2987 enrich + 783 chat-enrich + 880 discovery = ~4650 RPC дали 129 flood-wait событий, 2628 сек сна. Это укладывается в 90-минутный budget, но 30-минутный default НЕ достаточен.

### Known data asymmetries

API и JSON-экспорт **не совпадают** по адресной книге — это структурная особенность, не баг. На реальном дампе (диагностика через `scripts/telegram-diff-contacts.mjs`, нормализация через `libphonenumber-js` к E.164, регион `RU`):

| Источник | Размер | Что это |
|---|---|---|
| JSON `contacts.list` (|A|) | 2877 уникальных phone | Локальная адресная книга Telegram на момент экспорта (~2911 raw, минус 33 unparseable + 1 дубликат) |
| API `contacts.GetContacts` (|B|) | 1967 уникальных phone + 141 без phone | То что Telegram-сервер сейчас считает моими контактами (некоторые с `phone:null` из-за приватности) |
| `A ∩ B` | 1864 | Идеальный merge-сигнал: совпадает по phone в обоих источниках |
| `A \ B` (только JSON) | 1013 | Контакты были в моей адресной книге, но сейчас не в API |
| `B \ A` (только API) | 103 | Контакты есть в API, но не в локальной адресной книге |

**Интерпретация A \ B (1013):**
- 80% — RU-номера, остальные распределены по миру (AE, VN, UA, ID, TW…)
- Пик добавлений в 2018 году (534 контакта — старый период активного нетворкинга)
- 72% имеют только `first_name` (без `last_name`) — типичная русская «ФИО целиком в first_name» картина
- Лишь 2 контакта без имени совсем
- Наиболее вероятная природа: либо **удалённые/деактивированные TG-аккаунты** (но в локальной адресной книге Telegram запись осталась), либо **phone-only записи** для людей которые в TG не зарегистрированы. Без `user_id` (его и не было) — мы их можем сохранить только как Contact+PhoneNumber, без identity.

**Интерпретация B \ A (103):**
- 88/103 (85%) имеют `@username` — это люди которых я знаю по handle'у, не по номеру
- 60/103 (58%) — `mutual_contact=true` — близкий круг
- 0/103 удалённых — все живые аккаунты
- Природа: **implicit contacts** — люди с которыми у меня есть взаимодействие через Telegram (чаты, mutual contacts), но я их формально не добавлял через «Add Contact» с устройства. Поэтому в JSON `contacts.list` они не попали.

**Design rule для Дня 3-А:**

1. **API — source of truth для identity**. Все 1967+141 контактов из API создают `ContactIdentity(source='telegram', sourceId=user_id)`. Merge с vCard идёт по нормализованному `phone`, как и планировалось.
2. **JSON `contacts.list` — supplement для исторических**. 1013 записей из `A \ B` импортируются как `Contact + PhoneNumber` без `ContactIdentity` (потому что user_id у них нет и не будет). Это «телефонная книга» которая обогащает контакт-граф, но не претендует на реальный Telegram-аккаунт.
3. **141 API-контакта без phone** импортируются по `user_id`, merge с vCard невозможен (нет общего ключа); потом могут быть смерджены вручную в Дне 9.
4. **JSON `chats.list` + messages — для Дня 3-Б** (как и планировалось), полагается на identity уже из 3-А.
